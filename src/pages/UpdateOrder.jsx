...
import { Container, TextInput, Textarea, Select, Button, Title, Group, Card, Loader } from "@mantine/core";
import { getOrder, updateOrderItem, updateOrder, getStatuses } from "../services/Api";
...
export default function UpdateOrder() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [orderData, statusData] = await Promise.all([getOrder(id), getStatuses()]);
    ...
    const handleUpdateButtonClick = async () => {
        try {
            await Promise.all(updatedOrderItems.map(item => updateOrderItem(item.id, { quantity: item.quantity })));
    ...
    return (
        <Container>
            <Card shadow="sm" padding="lg" radius="md" withBorder>
                <Title order={2} align="center" mb="md">Edit Order</Title>
                ...
                <Select
                    label="Status"
                    data={statuses.map(status => ({ value: status.id.toString(), label: status.name }))}
                    value={order.status_id.toString()}
                    onChange={(value) => setOrder({ ...order, status_id: parseInt(value) })}
                />
                <Textarea
                    label="Address"
                    value={order.address}
                    onChange={(event) => setOrder({ ...order, address: event.target.value })}
                />
                <TextInput label="Total" value={order.total} disabled />

                <Title order={3} mt="md">Order Items</Title>
                {orderItems.map((item) => (
                    <Card key={item.id} shadow="sm" padding="lg" radius="md" withBorder mt="md">
                        ...
                        <TextInput
                            label="Quantity"
                            value={item.quantity}
                            onChange={(e) => handleUpdateOrderItemQuantity(item.id, e.target.value)}
                        />
